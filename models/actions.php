...
class actions_class extends AWS_MODEL
{
	...
	public function get_favorite_tag_action_list($tag, $uid, $limit)
	{
		if (!$uid)
		{
			return false;
		...
		foreach ($action_list as $key => $val)
		{
			...
			{
				if ($answer_info = $action_list_answers[$answer_id])
				{
					$action_list[$key]['answer_info'] = $answer_info;
				}
						
				if ($action_list[$key]['answer_info']['has_attach'])
				{
					$action_list[$key]['answer_info']['attachs'] = $answer_attachs[$answer_id];
				}
			}
			
			$action_list[$key]['question_info'] = $action_list_question_info[$question_id];
		}
