...
class answer_class extends AWS_MODEL
{
	...
	public function get_answer_list_by_question_id($question_id, $limit = 20, $where = null, $order_by = null)
	{
		...
		$sql = "SELECT answer.*, users.user_name, users.verified, users.group_id, users.url_token, attrib.* FROM " . $this->get_table('answer') . " AS answer LEFT JOIN " . $this->get_table('users') . " AS users ON answer.uid = users.uid LEFT JOIN " . $this->get_table('users_attrib') . " AS attrib ON answer.uid = attrib.uid WHERE answer.question_id IN(" . $question_id . ")";
