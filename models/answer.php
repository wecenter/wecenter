			...
			foreach ($at_users as $user_id)
			{
				if ($user_id != $uid)
				{
					...
					if ($weixin_user = $this->model('openid_weixin')->get_user_info_by_uid($user_id))
					{
						...
						$this->model('weixin')->send_text_message($weixin_user['openid'], $answer_user['user_name'] . " 在问题 [" . $question_info['question_content'] . "] 的答案评论中提到了您", $this->model('openid_weixin')->redirect_url('/m/question/' . $question_id . '?answer_id=' . $answer_info['answer_id'] . '&single=TRUE'));
					}
				}
			}
		}
