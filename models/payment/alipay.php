...
class payment_alipay_class
{
	public function createSign($out_trade_no, $subject, $total_fee, $extra_common_param = null)
	{
		$parameter = array(
			...
			"partner" => trim(AWS_APP::config()->get('alipay')->partner), 
			"_input_charset" => 'utf-8', 
			"seller_email" => trim(AWS_APP::config()->get('alipay')->seller_email), 
		...
		return $this->buildMysign($para_sort, AWS_APP::config()->get('alipay')->key);
	}
	...
	public function getReturnSign($para_temp)
	{
		...
		//生成签名结果
		$mysign = $this->buildMysign($para_sort, AWS_APP::config()->get('alipay')->key);
	...
	{
		$transport = strtolower(trim(AWS_APP::config()->get('alipay')->transport));
		$partner = trim(AWS_APP::config()->get('alipay')->partner);
		$veryfy_url = '';
		if ($transport == 'https')
		{
	...
	{
		if ($alipay_callback = curl_get_contents('https://mapi.alipay.com/gateway.do?service=close_trade&sign=' . $this->createCloseTradeSign($out_order_no, $trade_no) . '&order_no=' . $trade_no . '&out_order_no=' . $out_order_no . '&partner=' . AWS_APP::config()->get('alipay')->partner . '&_input_charset=' . AWS_APP::config()->get('alipay')->input_charset . '&sign_type=MD5'))
		{
