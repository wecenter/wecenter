...
class question_class extends AWS_MODEL
{
	...
	public function get_question_list_by_topic_ids($topic_ids, $category_id = null, $answer_count = null, $order_by = 'question_id DESC', $is_recommend = false, $page, $per_page)
	{
		...
		$result_cache_key = 'question_list_by_topic_ids_' . implode('_', $topic_ids) . '_' . md5($answer_count . $category_id . $order_by . $page . $per_page);
	 ...
	 * @return array
	 */
	public function get_question_topic_by_question_id($question_id)
	{
		$result = $this->get_question_topic_by_question_ids(array(
			$question_id
		));
	...
	public function get_question_topic_by_question_ids($question_ids)
	{		
		if (!is_array($question_ids) OR sizeof($question_ids) == 0)
		{
			return false;
		...
		foreach ($question_topic AS $key => $val)
		{
		...
		foreach ($question_topic AS $key => $val)
		{
			$result[$val['question_id']][] = $topics_info[$val['topic_id']];
		}
