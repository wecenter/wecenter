...
class topic_class extends AWS_MODEL
{
	...
	{
		$best_answer_users = AWS_APP::cache()->get('best_answer_users_' . md5($topic_id . $uid . $limit));
		
		if (is_array($best_answer_users))
		{
		...
		if ($uid_list = $this->fetch_all('reputation_topic', 'reputation > 0 AND topic_id = ' . intval($topic_id), 'reputation DESC', $limit))
		{
			foreach ($uid_list AS $key => $val)
			{
				...
				$best_answer_users[$val['uid']]['agree_count'] = $val['agree_count'];
				$best_answer_users[$val['uid']]['thanks_count'] = $val['thanks_count'];
			}
		...
		AWS_APP::cache()->set('best_answer_users_' . md5($topic_id . $uid . $limit), $best_answer_users, get_setting('cache_level_normal'));
	...
	public function get_topic_action_list($topic_ids, $limit, $best_answer = false)
	{
		...
		if (!$action_list = AWS_APP::cache()->get('topic_action_list_' . md5($topic_ids . $limit . $best_answer)))
		{
			...
			{
				$question_ids = $this->model('topic')->get_question_ids_by_topics_ids($topic_ids, $limit);
			}
			...
			else if (!$action_list = ACTION_LOG::get_actions_distint_by_where("(associate_type = " . $associate_type . " AND associate_id IN (" . implode($associate_id, ",") . ") AND associate_action IN(" . implode(',', $associate_action) . "))", null))
			{
				$action_list = array();
			}
