		...
		switch ($action)
		{
			...
			case ACTION_LOG::ADD_TOPIC : //'添加话题',
				if ($topic_info && $user_name)
				{
					...
					{
						$action_str = '<a href="' . $user_url . '" ' . $user_tip . '>' . $user_name . '</a> ' . AWS_APP::lang()->_t('将该问题添加到') . ' <a href="topic/' . $val['url_token'] . '" ' . $topic_tip . '>' . $topic_info['topic_title'] . '</a> ' . AWS_APP::lang()->_t('话题');
