...
class core_image
{
	...
	public function resize()
	{
		...
		{
			show_error('Core_Image: Source file not exists: ' . $this->source_image);
		}
		...
		{
			default:
				show_error('Core_Image: Can\'t detect output image\'s type.');
			break;
	...
	private function imageProcessGD()
	{
		...
		{
			show_error('Core_Image: Function not exists for output: ' . $func_output);
		}
		...
		{
			show_error('Core_Image: Function not exists for output: ' . $func_create);
		}
		...
		else if ($this->option == IMAGE_CORE_OP_OUTPUT)
		{
			...
			{
				show_error('Core_Image: HTTP already sent, can\'t output image to browser.');
			}
