	...
	$.extend({
		...
		I:{
			...
			searchs: function(data){
				...
				//
					callback = {
						elem: '<div id="search_global"><p class="s">' + _t('输入关键字进行搜索') + '...</p>'+
							  (G_USER_ID == 0 ? '' : '<p class="qs"><a class="i_green_but i_right" href="javascript:;" onclick="$.startQs({category_enable:'+data.attr('data-category_enable')+',search:\''+(el.val()== _t('搜索问题、话题或人') ? '': el.val())+'\'});$(\'#data_lsit2\').hide();">' + _t('发起问题') + '</a></p>')+
							  '</div>'
					};
			...
			search_global:function(){
				...
				var resultArr = [],
					es= true,
				    flg = {
						f0:'<p class="s">' + _t('输入关键字进行搜索') + '...</p>',
						f1:'<p class="s">' + _t('请输入两个以上关键字') + '...</p>',
						f2:'<p class="s" data-url="nothing">' + _t('没有找到相关结果') + '...</p>',
						f3:'<p class="s">' + _t('请稍后, 正在加载') + '...</p>',
						elem: (G_USER_ID!=0 ? '<p class="qs" data-url="startQs"><a class="i_green_but i_right" href="javascript:;" onclick="$.startQs({category_enable:'+el.attr('data-category_enable')+',search:\''+val+'\'});$(\'#data_lsit2\').hide();">' + _t('发起问题') + '</a><b class="i_gltxtHide">'+val+'</b></p>':''),
				...
				switch($.trim(val).length){
					...
					default :
						if(val != _t('搜索问题、话题或人')){
							...
							var elSval = '<p data-message="'+val+'" data-url="搜索"><a href="javascript:;" onclick="$(\'#global_search_btns\').click();">' + _t('搜索') + ': <b>'+val+'</b></a></p>';
										...
										$(result).each(function(ix, element) {
											...
											if(ix < 10 ){
												switch(len){
													//问题
													case 1:
														elemArr = '<p data-message="'+idex.name+'" data-url="'+G_BASE_URL+'/'+idex.url+'">'+(idex.detail.best_answer <= 0 ? '' : '<span class="best_answer" title="' + _t('最佳回复') + '"></span>')+
																  '<a class="s_txt_qs i_gltxtHide" href="'+G_BASE_URL+'/'+idex.url+'"  title="'+idex.name+'">'+(idex.name.replace(new RegExp(val,"ig"),'<b>'+val+'</b>'))+'</a>'+
																  '<span class="xs i_cl_02 i_gltxtHide">'+(idex.detail.answer_count == null ? 0 : idex.detail.answer_count)+' ' + _t('个回复') + '</span></a></p>';
													break;
													...
													case 2:
														elemArr = '<p data-message="'+idex.name+'" data-url="'+G_BASE_URL+'/'+idex.url+'"><a class="i_glotopic user_msg" href="'+G_BASE_URL+'/'+idex.url+'" data-message="&uid='+idex.sno+'&card=topic">'+idex.name+'</a>'+
														          '<span class="xs i_cl_02 i_gltxtHide">'+(idex.detail.discuss_count == null ? 0 : idex.detail.discuss_count)+' ' + _t('个问题') + '</span></p>';
													break;
													...
													case 3:
														elemArr = '<p data-message="'+idex.name+'" data-url="'+G_BASE_URL+'/'+idex.url+'"><a href="'+G_BASE_URL+'/'+idex.url+'" class="user_header">'+
																  ...
																  '<span class="xs i_cl_02 i_gltxtHide">'+(idex.detail.signature == null || idex.detail.signature =='' ? _t('暂无介绍') : idex.detail.signature)+'</span></p>';
													break;
													default:
												}
											}
			...
			msg:function( x,el ){
				...
				.toggle('fast',function(){
					$(obj).attr('title')==_t('展开') + ' »' ? 
					$(obj).attr('title',_t('收起') + ' »').html(_t('收起')) : 
					$(obj).attr('title',_t('展开') + ' »').html(_t('展开'));
				});
			}, //end msg
			...
			privateLetters: function (flg){
				...
				if($.trim($(flg).val()) == _t('搜索用户') || $.trim($(flg).val()).length <= 0){
				
					$.dataList({flgs:flg,html:'<p style="height:30px; line-height:30px;padding:0 5px;font-size:12px;">' + _t('输入关键字开始搜索') + '...</p>'});
			...
			searchUser:function(flg,e,y){
				...
				if($.trim($(flg).val()).length <=1 ){
					$.dataList({flgs:flg,html:'<p class="s">' + _t('请输入两个以上关键字') + '...</p>'})
				}else{
					$.ajax({
						type:'GET',
						...
						dataType:'json',
						beforeSend: function(){
							$('#data_lsit2').show().html('<p class="s">' + _t('请稍后, 正在加载') + '...</p>');
						},
						success: function(result){
							if(result&& result.length >0) {
								...
								$.dataList({flgs:flg,html:'<p class="s">' + _t('未找到相关用户') + '...</p>'})
							}
		   ...
		   elemetsClcik :function( flgs ){
			  ...
			  if($('#privateLetter_user_id').length > 0 ){
				...
				.html(elem.attr('data-value')+'<em onclick="$.I.plCilckAddvalue();" class="q_editor i_small">' + _t('编辑用户') + '</em>').fadeIn('slow');
				  ...
				  .html(elem.attr('data-value')+'<em onclick="$.I.plCilckAddvalue();" class="q_editor i_small">' + _t('编辑用户') + '</em>')
		   ...
		   plCilckAddvalue:function(){
			...
			click_eve: function(options){
				...
				.addClass('s_xs i_pas')
				.html(
					'<a class="s_zt i_prl '+(fls ? sClass :'')+'" href="javascript:;" onClick="$.I.addCast(this,\''+flgs.username+'\',\''+flgs.answer_id+'\')" data-id="'+flgs.uid+'"><span class="i_tips_black i_pas i_hide"><em></em>' + _t('赞同回复') + '</span></a>'+
					'<em title="' + _t('被赞同 %s 次', flgs.appNum) + '" data-number="'+flgs.appNum+'" class="s">'+flgs.appNum+'</em>'+
					'<a class="s_fd i_prl '+(fls ? '' : (fls ==null ? '' :sClass))+'" href="javascript:;" onClick="$.I.oppCast(this,\''+flgs.username+'\',\''+flgs.answer_id+'\')" data-id="'+flgs.uid+'"><span class="i_tips_black i_pas i_hide"><em></em>' + _t('对回复持反对意见') + '</span></a>'
				)
			...
			selectd: function(options){
				...
				if($(flgs.listId).find('div.txtoverflow').length > 0){
					...
					$.ajax({
						...
						dataType:"json",
						beforeSend: function(){
							flgs.listId.fadeIn('slow').html('<p class="s"><a href="javascript:;">' + _t('请稍后, 正在加载') +'...</a></p>');
						},
						success: function(result){
							if(result&& result.length >0) {
								$(result).each(function(index, element) {
			...
			favorite:function(answer_id){
					var flgElem = $('<div/>')
						...
						$.get(G_BASE_URL + '/favorite/ajax/get_favorite_tags/', function (result) {
							html_data = '<form action="' + G_BASE_URL + '/favorite/ajax/update_favorite_tag/" method="post" onsubmit="return false;">'+
								'<div class="qs_txtare ">' + _t('添加话题标签') + ': '+
									'<input type="hidden" name="answer_id" value="' + answer_id + '" />'+
									'<input class="i_glotxtClass" style="width:200px;height:30px;line-height:30px;" name="tags" id="add_favorite_tags" />'+
								'</div>'+
								'<div class="f_topic" id="add_favorite_my_tags">' + _t('常用标签') + ': ';
							...
							html_data += '</div>'+
								...
								'<div class="qs_txtare i_tr">'+
									'<a onclick="$.closeDefault(this);" href="javascript:;">' + _t('取消') + '</a>&nbsp;&nbsp;&nbsp;&nbsp;'+
									'<a class="i_green_bt2 i_txtmiddle" onclick="ajax_post($(this).parents(\'form\'), _ajax_post_i_alert_processer)" href="javascript:;">' + _t('确定') + '</a>'+
								'</div></form>';
		...
		*---------------------------------------------*/
		topic:{
			...
			editor_topic:function(options){
				...
				var pop = options.pop ? '_pop': (options.answer_id != null ? options.answer_id : '' ),
					...
					topicEditor:$('#editor_input_handle_i_PublicTopic'+pop),
							txt:options.make ? _t('创建新的话题') + '...': _t('创建或搜索添加新话题') + '...', //make 为布尔值
				...
				if(option.topicEditor.length > 0){
					...
					if (G_CAN_CREATE_TOPIC || option.create)
					{
						add_button = '&nbsp;<a class="i_green_bt2" id="establish_topic'+pop+'" href="javascript:;" onclick="$.topic.establish_topic('+option.f+','+option.answer_id+',\''+pop+'\');">' + _t('添加') + ' »</a>';
					}
					
					$('<div/>').addClass('editor_input i_hide')
					.attr('id','editor_input_handle_'+option.topicHolder.attr('id'))
					.html('<input type="text" id="editor_input'+pop+'" maxlength="13" onfocus="$.focus(this,\''+option.txt+'\');" value="'+option.txt+'" class="i_glotxtClass" /> ' + add_button + ' &nbsp;<a class="i_gray_bt2" '+
					'href="javascript:;" onclick="$.topic.countermand_topic('+option.f+',\'#editor_input_handle_'+option.topicHolder.attr('id')+'\',\'#'+option.topicHolder.attr('id')+'\','+option.answer_id+');">' + _t('取消') + '</a>')
			...
			establish_topic:function(txt,qsID,pop){
				...
				var tx = txt ? _t('创建新的话题') + '...': _t('创建或搜索添加新话题') + '...';
				var el,eleminput;
				...
				if($.trim(eleminput.val()).length == 0 || eleminput.val() == tx){
					eleminput.focus();
					$.dataList({flgs:eleminput,html:'<p class="s">' + _t('请输入话题标题') + '</p>'});
					
					return ;
				}else if($.trim(eleminput.val()).length >12 ){
					eleminput.focus();
					$.dataList({flgs:eleminput,html:'<p class="s">' + _t('话题标题限制 12 个字符以内') + '...</p>'});
					return ;
				}else{
			...
			elemClciks: function (flg,txt,val,qsID,pop){
				...
				if(val == 'input'){ //input添加话题
					...
					if(flgval ==''){
						flg.val('').focus();
						$.dataList({flgs:flg,html:'<p class="s">' + _t('请输入话题标题') + '...</p>'});
						return;
				...
				if(txt){
					 ...
					 if(trues){
						 $('<a/>')
						  ...
						  .html(flgval+'<em class="handle i_pas" onclick="$.topic.deleted_topic(this,event,true,'+qsID+',\''+pop+'\'); return false;">×</em><input type="hidden" name="topics[]" value="'+flgval+'" />')
						  .appendTo(topicSpan)
						  .fadeIn('slow');
						  editor_input.val('').focus();
					 }else{
					 	 $.dataList({flgs: editor_input.val('').focus(),html:'<p class="s">' + _t('同名话题已存在') + '...</p>'});
					 }
							  ...
							  success: function(result){
								  if(result.rsm){
									  ...
									  if(trues){
										  ...
										  .css('display','none')
										  .addClass('i_glotopic i_prl pd')
										  .html((val == 'input' ? flgval : flg.text())+'<em class="handle i_pas" onclick="$.topic.deleted_topic(this,event,false,'+qsID+',\''+pop+'\'); return false;">×</em>')
										  .appendTo(topicSpan)
										  .fadeIn('slow');
										  $.dataList( {flgs:editor_input.val('').focus(),html:'<p class="s">' + _t('话题添加成功') + '...</p>'});
									  }else{
									  	 $.dataList( {flgs:editor_input.val('').focus(),html:'<p class="s">' + _t('同名话题已存在') + '...</p>'});
									  }
			...
			topicKeyEve:function( flg,txt,qsID,pop){
				...
				var el = $(flg),myFunction,
					defaultic = {
						tx:txt ? _t('创建新的话题') + '...': _t('创建或搜索添加新话题') + '...',
				...
				if($.trim(el.val()).length == 0 || el.val() == defaultic.tx){
					...
					$.dataList({flgs:el,html:'<p class="s">' + _t('请输入两个以上关键字') + '...</p>'});
					return;
				}else if($.trim(el.val()).length > 12){
					$.dataList({flgs:el,html:'<p class="s">' + _t('话题标题限制 12 个字符以内') + '...</p>'});
					return;
				}else{
					if($.trim(el.val()).indexOf(',') >=0 || $.trim(el.val()).indexOf('，') >= 0){
					 ...
					 //if(!txt){
						 $.ajax({
							...
							dataType:'json',
							beforeSend: function(){
								$.dataList({flgs:el,html:'<p class="s">' + _t('请稍后, 正在加载') + '...</p>'});
							},
							success: function(result){
								var results;
								if(result&& result.length >1) {
									...
									if(result[0].exist == 0){
										$.dataList({flgs:el,html:'<p class="s">' + _t('未找到相关话题') + '...</p>'});
									}else{
										$('#data_lsit2').hide();
									}
								}
		...
		question_reset:function(){
			...
			if(el.length > 0 ){
				el.changeElement(function(){
					if($.trim(el.val()).length == 0 || el.val() == _t('搜索问题')){
						...
						$.ajax({
							...
							success: function(s){
								if(s && s.length > 0){
									...
									$.dataList({flgs:el,html:'<p class="s">' + _t('没有找到相关结果') + '...</p>'});
								}
							},
							complete: function(){
							  ...
							  if(s && s.errno ==1){
								  ...
								  $('<li/>')
									.html('<a href="javascript:;" class="pic_xa i_left">'+
										  ...
										  '<a class="i_gray_bt2 i_right" href="javascript:;" onclick="$.disinvite_user('+flgs.uid+',this,\''+flgs.user+'\')">' + _t('取消邀请') + '</a>'+
							  ...
							  if(s && s.errno ==1){
								  $('<li/>')
									...
									.html('<a href="javascript:;" class="pic_xa i_left">'+
										  ...
										  '<a class="i_gray_bt2 i_right" href="javascript:;" data-id="'+flg.attr('data-id')+'" onclick="$.disinvite_user('+flg.attr('data-id')+',this,\''+flg.attr('data-value')+'\')">' + _t('取消邀请') + '</a>'+
						  ...
						  if(result && result.length > 0){
							 $(el).unbind('keyup'),flx = true;
							  $(result).each(function(index, element) {
								  ...
								  index <= 8 ? 
								  flg.elemsArr.push('<p class="'+(index==0 ? 'closex':'')+'"><a href="'+G_BASE_URL+'/'+elResult.url+'" title="'+elResult.name+'">'+(elResult.name.replace(new RegExp(val,"ig"),'<b>'+val+'</b>'))+(index==0?'<em class="e" onclick="$.mpopClose(this,event);">x</em>':'')+'</a></p>') :'';
							  })
							  $.dataList({
								  flgs:el,
		...
		uploadPicture:function(){
			var flgElem= $('<div/>')
				...
				.attr('id','i_gloBoxl')
				.html('<div class="i_glotitle">'+
						'<h3 class="i_prl"><p class="qs_bold i_white">' + _t('插入图片') + '</p>'+
						'<a onclick="$.closeDefault(this);" class="i_right i_pas i_small i_closed" href="javascript:;">' + _t('关闭') + '</a></h3>'+
						'</div>'+
						'<div class="i_gloDiv" id="i_globalContant">'+
						'<form id="addTxtForms"><div class="qs_txtare" style="padding:0!important;">' + _t('图片链接地址') + ': '+
							'<div class="add_label ">'+
								'<input type="text" name="imgsUrl" class="i_glotxtClass upload_txt"  value="http://" onfocus="$.focus(this,\'\');"/>'+
							'</div>'+
						'</div>'+
						'<div class="qs_txtare">' + _t('图片说明') + ':'+
							'<div class="add_label ">'+
						...
						'<div class="qs_txtare i_tr">'+
							'<a onclick="$.closeDefault(this);" href="javascript:;" id="closeDefault">' + _t('取消') + '</a>&nbsp;&nbsp;&nbsp;&nbsp;'+
							'<a class="i_green_bt2 i_txtmiddle" id="qsSubmite_form1"  href="javascript:;" onclick="$.addTextpicTure()">' + _t('确定') + '</a>'+
						'</div>'+
