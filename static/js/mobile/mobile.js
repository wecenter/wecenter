$(function() {
	...
	$('#wm-top-nav-profile').click(function() {
		$('.wm-top-nav-popup').hide();
		$('.wm-top-nav-profile').toggle();
	});

	$('#wm-top-nav-notic').click(function() {
		$('.wm-top-nav-popup').hide();
		$('.wm-top-nav-notic').toggle();
	});

	//关闭按钮
	$('.slide-close').click(function() {
		$(this).parents('.wm-top-nav-popup').hide();
	});

	//点击下拉菜单外得地方隐藏
	$(document).click(function(e) {
		if ($(e.target).closest('#wm-top-nav-notic, #wm-top-nav-profile, .wm-top-nav-popup').length == 0)
		{
			$('.wm-top-nav-popup').hide();
		}
	});
