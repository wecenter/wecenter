	...
	$(element).click(function()
	{
		...
		$('.aw-topic-box-selector .add').click(function()
		{
			switch (data_type)
			{
				...
				case 'question' :
					var _this = $(this);
					$.post(G_BASE_URL + '/topic/ajax/save_topic_relation/', 'type=question&item_id=' + data_id + '&topic_title=' + $(this).parents('.aw-topic-box-selector').find('.aw-topic-input').val(), function(result)
					{
						if (result.errno == 1)
						{
				...
				case 'article' :
					var _this = $(this);
					$.post(G_BASE_URL + '/topic/ajax/save_topic_relation/', 'type=article&item_id=' + data_id + '&topic_title=' + $(this).parents('.aw-topic-box-selector').find('.aw-topic-input').val(), function(result)
					{
						if (result.errno == 1)
						{
							_this.parents('.aw-topic-edit-box').find('.aw-topic-box').prepend('<span class="aw-topic-name" data-id="'+ result.rsm.topic_id +'"><a>' + _this.parents('.aw-topic-box-selector').find('.aw-topic-input').val() + '</a><a><i>X</i></a></span>');
		...
		$('.aw-question-detail-title .aw-topic-edit-box .dropdown-list ul li').click(function()
		{
			...
			switch (data_type)
			{
				...
				case 'article' :
					$.post(G_BASE_URL + '/question/ajax/save_topic/article_id-' + $(this).parents('.aw-topic-edit-box').attr('data-id'), 'topic_title=' + $(this).text(), function(result)
					{
						if (result.errno == 1)
						{
							$(element).parents('.aw-topic-edit-box').find('.aw-topic-box').prepend('<span class="aw-topic-name" data-id="' + result.rsm.topic_id + '"><a>' + _this.text() + '</a><a href="#"><i>X</i></a></span>');
