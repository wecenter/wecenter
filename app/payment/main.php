...
class main extends AWS_CONTROLLER
{
	...
	{    	
		...
		TPL::assign('partner', AWS_APP::config()->get('alipay')->partner);
		TPL::assign('seller_email', AWS_APP::config()->get('alipay')->seller_email);
    	...
    	if (is_mobile())
    	{
			...
			$html_text = $this->model('payment_aliwap')->buildRequestHttp(array(
					...
					"partner" => AWS_APP::config()->get('alipay')->partner,
					"sec_id" => AWS_APP::config()->get('alipay')->sign_type,
					"format"	=> 'xml',
					"v"	=> '2.0',
					"req_id"	=> $req_id,
					"req_data"	=> '<direct_trade_create_req><notify_url>' . $notify_url . '</notify_url><call_back_url>' . $call_back_url . '</call_back_url><seller_account_name>' . AWS_APP::config()->get('alipay')->seller_email . '</seller_account_name><out_trade_no>' . $order_id . '</out_trade_no><subject>' . $order_name . '</subject><total_fee>' . $amount . '</total_fee><merchant_url>' . get_setting('base_url') . '</merchant_url></direct_trade_create_req>',
					"_input_charset"	=> AWS_APP::config()->get('alipay')->input_charset
			));
