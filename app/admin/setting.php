...
class setting extends AWS_CONTROLLER
{
	...
	public function test_email_setting_action()
	{
		...
		$sendmail_result = $core_mail->send_mail($_POST['from_email'], get_setting('site_name'), $_POST['test_email'], $_POST['test_email'], get_setting('site_name') . ' - ' . AWS_APP::lang()->_t('邮件服务器配置测试'), AWS_APP::lang()->_t('这是一封测试邮件，收到邮件表示邮件服务器配置成功'));
