...
class edm extends AWS_CONTROLLER
{
	...
	public function add_group_action()
	{
		...
		if (trim($_POST['title']) == '')
		{
			H::redirect_msg(AWS_APP::lang()->_t('请填写用户群名称'));
		}
	...
	public function add_task_action()
	{
		if (trim($_POST['title']) == '')
		{
			H::redirect_msg(AWS_APP::lang()->_t('请填写任务名称'));
		}
					
		if (trim($_POST['subject']) == '')
		{
			H::redirect_msg(AWS_APP::lang()->_t('请填写邮件标题'));
		}
					
		if (intval($_POST['usergroup_id']) == 0)
		{
			H::redirect_msg(AWS_APP::lang()->_t('请选择用户群组'));
		}
		...
		H::redirect_msg(AWS_APP::lang()->_t('任务建立完成'), get_setting('base_url') . '/?/admin/edm/tasks/');
