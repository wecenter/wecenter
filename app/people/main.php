...
class main extends AWS_CONTROLLER
{
	...
	public function list_action()
	{
		...
		if ($this->user_info['city'] AND $_GET['sort_key'] == 'city')
		{
			$where[] = "`city` = '" . $this->model('account')->quote($this->user_info['city']) . "'";
		}
		
		if ($users_list = $this->model('account')->get_users_list(implode('', $where), calc_page_limit($_GET['page'], get_setting('contents_per_page')), true, false, $sort_key . ' DESC, uid DESC'))
		{
			...
			TPL::assign('pagination', AWS_APP::pagination()->initialize(array(
				'base_url' => get_js_url('/people/list/sort_key-' . $_GET['sort_key']), 
